...
class OWIRSRWSpectrum(SRWWavefrontViewer):
    ...
    description = "SOLEIL SRW Source: Source Spectrum"
    ...
    spe_photon_energy_min = Setting(0.01)
    spe_photon_energy_max = Setting(1.01)
    spe_photon_energy_points=Setting(100)
    ...
    def getYTitles(self):
        if not self.received_light_source  is None and (isinstance(self.received_light_source, SRWBendingMagnetLightSource) or isinstance(self.received_light_source, SRWIRBendingMagnetLightSource)):
        ...
        if not data is None:
            try:
                if isinstance(data, SRWData):
                    ...
                    if isinstance(self.received_light_source, SRWIRBendingMagnetLightSource) or isinstance(self.received_light_source, SRWIRBendingMagnetLightSource):
